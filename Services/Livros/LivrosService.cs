...
{
    public class LivrosService : ILivrosInterface
    {
        ...
        public async Task<ResponseModel<List<LivroModel>>> BuscarLivroPorIdAutor(int IdLivro)
        {
            ...
            try
            {
                ...
                var livro = await _context.Livros.Include(l => l.Autor).Where(LivroBanco => LivroBanco.Autor.Id == IdLivro).ToListAsync();
                if (livro == null)
                {
                    resposta.Mensagem = "Nenhum Autor vinculado há este livro";
                    return resposta;
                }
        ...
        public async Task<ResponseModel<List<LivroModel>>> CriarLivro(LivrosCriacaoDto livrosCriacaoDto)
        {
            ResponseModel<List<LivroModel>> resposta = new ResponseModel<List<LivroModel>>();
            try
            {
                var autor = await _context.Autores.FirstOrDefaultAsync(autorBanco => autorBanco.Id == livrosCriacaoDto.Autor.Id);
                if (autor  == null)
                {
                ...
                {
                    Titulo = livrosCriacaoDto.Titulo,
                    Autor = autor,
                };
